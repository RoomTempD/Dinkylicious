'--put on top of each page
'Dim Connection As OleDb.OleDbConnection = New OleDb.OleDbConnection(Saved.Database_String)
'Dim ds As New DataSet
'Dim da As New OleDb.OleDbDataAdapter

'--to select records
'Connection.Open()
'da.SelectCommand = New OleDb.OleDbCommand("SELECT SIGNINOUT.EMP_NUM, EMPLOYEE.EMP_NAME FROM EMPLOYEE INNER JOIN SIGNINOUT ON EMPLOYEE.EMP_NUM = SIGNINOUT.EMP_NUM WHERE SIGNOUT_TIME IS NULL AND SIGNIN_TIME IS NOT NULL AND (JOB_NUM = 1 OR JOB_NUM = 6) ORDER BY SIGNINOUT.EMP_NUM", Connection)
'Connection.Close()
'da.Fill(ds, "Table")

'--to iterate through records
'For Each DataRow As DataRow In ds.Tables(0).Rows
'   DataRow("EMP_NUM").ToString()
'Next

'--to display and format in DGV
'DGV.DataSource = ds.Tables("Table")
'DGV.Columns("ITEM_NUM").Visible = False
'DGV.Columns("ITEM_NAME").Width = 600

'--to get back single value
'ds.Tables(0).Rows(0).Item(0)

'--to execute non-query
'Dim newcommand As New OleDb.OleDbCommand("DELETE FROM Specials where ITEM_NUM = 78", Connection)
...
'   dgvEmp.SelectedRows(0).Cells(0).Value
'End If
...
Module Saved
    ...
    Public Function getDateTime(ByVal sentDate As Date) As DateTime

    End Function

    'Public Function GetOrderNumber(ByVal TicketNumber As Integer, ByVal GuestNumber As Integer) As Integer
    '    Return GetValue("SELECT ORDER_NUM FROM OPEN_TICKET WHERE TICKET_NUM = " & TicketNumber & " AND GUEST_NUM = " & GuestNumber)
    'End Function
